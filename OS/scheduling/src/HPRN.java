...
public class HPRN {
	...
	public HPRN(int numOfProcesses, ArrayList<Process> processes, int output_option, Scanner rand){
	...
	public String toString(){
		...
		if (output_option == 1){ // print  out the simpler results
			...
			while (terminated != sorted_processes.size()){
				...
				if (current_process == null && !readyToRun.isEmpty()){
					Process hprn = readyToRun.get(0);
					for (int i = 0; i < readyToRun.size(); i++){
						if ((double)(cycle_counter - hprn.get_arrivalTime()) / Math.max(1,hprn.getTotalRanTime()) < ((double)cycle_counter - readyToRun.get(i).get_arrivalTime()) / Math.max(1, readyToRun.get(i).getTotalRanTime())){
							hprn = readyToRun.get(i);
						}
						else if ((((double)cycle_counter - hprn.get_arrivalTime()) / Math.max(1,hprn.getTotalRanTime()) == ((double)cycle_counter - readyToRun.get(i).get_arrivalTime()) / Math.max(1, readyToRun.get(i).getTotalRanTime()))){
							//System.out.println(cycle_counter);
							if (hprn.get_arrivalTime() > readyToRun.get(i).get_arrivalTime()){
								hprn = readyToRun.get(i);
							}
							else if (hprn.get_arrivalTime() == readyToRun.get(i).get_arrivalTime() && i > 0){
								if (sorted_processes.indexOf(hprn) > sorted_processes.indexOf(readyToRun.get(i))){
									hprn = readyToRun.get(i);
								}
							}
						}
					}	
					current_process = hprn;
				}
				...
				if (current_process != null){
					...
					if (current_process.getState().equals("running")){
						if (current_process.getRemaining() == 1){						
							...
							current_process.increment_total_running();
						}
					}
		...
		output += "The scheduling algorithm used was Highest Penalty Ratio Next\n";
		float cpu_total = 0;
		float thruput;
		float tat_total = 0;
