...
public class FCFS {
	...
	public FCFS(int numOfProcesses, ArrayList<Process> processes, int output_option, Scanner rand){
	...
	public String toString(){
		...
		if (output_option == 1){ // print out the simpler results	
			...
			while (terminated != sorted_processes.size()){
				...
				if (current_process == null && !readyToRun.isEmpty()){
					Process fcfs_priority = readyToRun.get(0);
					for (int i = 0; i < readyToRun.size(); i++){
						if (fcfs_priority.getWaitingTime() < readyToRun.get(i).getWaitingTime()){
							fcfs_priority = readyToRun.get(i);
						}					
						else if (fcfs_priority.getWaitingTime() == readyToRun.get(i).getWaitingTime()){
							if (fcfs_priority.get_arrivalTime() > readyToRun.get(i).get_arrivalTime()){
								fcfs_priority = readyToRun.get(i);
							}
							else if (fcfs_priority.get_arrivalTime() == readyToRun.get(i).get_arrivalTime() && i > 0){
								if (sorted_processes.indexOf(fcfs_priority) > sorted_processes.indexOf(readyToRun.get(i))){
									fcfs_priority = readyToRun.get(i);
								}
							}
						}
					}
					current_process = fcfs_priority;
				}
		...
		output += "The scheduling algorithm used was First Come First Served\n\n";
		float cpu_total = 0;
		float io_total = 0;
		float thruput;
		float tat_total = 0;
